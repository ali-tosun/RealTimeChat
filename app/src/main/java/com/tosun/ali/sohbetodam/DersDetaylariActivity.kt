...
class DersDetaylariActivity : AppCompatActivity() {
        ...
        var urlTask = uploadTask.continueWithTask(object : Continuation<UploadTask.TaskSnapshot, Task<Uri>>{
            ...
            if (it.isSuccessful) {
               ...
               var referans= FirebaseDatabase.getInstance().reference
                ...
                referans.child(yeniOdevId)
                        .child("olusturulma_zamani")
                        .setValue(yuklenmeZamani)

                referans.child(yeniOdevId)
                        .child("yukleyen_kisi")
                        .setValue(yukleyen_id)

                Log.e("indirmelink",downloadUrl.toString())

            } else {
                Toast.makeText(this@DersDetaylariActivity, it.exception!!.message, Toast.LENGTH_LONG).show()
            }
        }
